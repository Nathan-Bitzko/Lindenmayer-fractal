...
threadQueue = [] #threading is to prevent the gui from freezing rather than for speed
lastThread = threading.Thread()
...
def runThreads():
    ...
    if len(threadQueue) > 0 and not lastThread.is_alive():
        lastThread = threadQueue.pop(0)
        lastThread.start()
    elif lastThread.is_alive():
        rules.after(20, runThreads)
